const StoreOneProductVideo = {
  ...
  buildVideo(video, type, autoplay, thumb) {
    let auto = autoplay ? 1 : 0;
    let mute = autoplay ? 1 : 0;

    if (type === "youtube") {
      let id = this.getYouTubeId(video);
      if (!id) return "";
  ...
  playVideo($slide) {
    ...
    let html = this.buildVideo(video, type, autoplay, thumb);
    if (!html) return;
  ...
  modifyVideoThumbs() {
    ...
    slides.each(function (index) {
      ...
      if (li.length && !li.find(".th-video-thumb-icon").length) {
        ...
        let iconType =
          typeof thVideoData !== "undefined" ? thVideoData?.icon : "outline";
        let iconColor =
          typeof thVideoData !== "undefined" ? thVideoData?.color : "#7388FFBA";
        let iconHtml = self.getVideoIcon(
          iconType || "outline",
          iconColor || "#7388FFBA",
        );
  ...
  bindEvents() {
    ...
    $(document).on("click", ".flex-next, .flex-prev", () => {
      this.onSlideChange();
    });

    // Ajax variation change fix
...
export default StoreOneProductVideo;
