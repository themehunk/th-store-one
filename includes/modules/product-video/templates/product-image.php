...
$g_ratio_custom = $settings['g_ratio_custom'] ?? '';
...
if (!is_array($th_enable_custom_poster)) {
    $th_enable_custom_poster = [];
}
...
if ($enable === 'yes' && ! empty($videos) && is_array($videos)) {
    ...
    foreach ($videos as $index => $video) {
        ...
        }/* SELF HOSTED VIDEO */ elseif ($type === 'upload') {
        ...
        } elseif ($final_aspect === 'custom') {
            $aspect_class = 'th-aspect-custom';

            if (! empty($g_ratio_custom)) {
                $aspect_style = '--th-g-aspect-ratio:' . esc_attr($g_ratio_custom) . ';';
            }
        }
        ...
        if ($type === 'youtube') {
            $source_class = 'th-video-youtube';
        } elseif ($type === 'vimeo') {
            $source_class = 'th-video-vimeo';
        } elseif ($type === 'upload') {
            $source_class = 'th-video-upload';
        }
        ?>
            <div class="woocommerce-product-gallery__image th-video-slide <?php
                echo esc_attr($aspect_class);
        ?> <?php echo esc_attr($source_class); ?>" style="<?php echo esc_attr($aspect_style); ?>"
