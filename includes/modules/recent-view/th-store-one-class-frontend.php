...
class Th_Store_One_Recent_View {
    ...
    public function __construct() {
        ...
        if (empty($modules['recent-view'])) return;
    ...
    public function track_product() {
        ...
        $viewed = isset($_COOKIE['th_recent_view'])
            ? explode('|', $_COOKIE['th_recent_view'])
            : [];
    ...
    if (empty($_COOKIE['th_recent_view'])) return [];

    $ids = array_filter(array_map('intval', explode('|', $_COOKIE['th_recent_view'])));
    if (is_product()) {
    ...
    $products = $this->get_products();
    if (empty($products)) return $content;

    ob_start();
    $this->render();
    $recent_html = ob_get_clean();

    if (empty($recent_html)) return $content;
