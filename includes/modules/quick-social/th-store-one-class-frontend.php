    ...
    private function should_display_rule($rule)
    {
        ...
        */
        ...
        switch ($trigger) {
            ...
            |--------------------------------------------------------------------------
            | Pages + Posts only
            | Excludes WooCommerce Product Pages
            */
            case 'all_pages':

                // Skip Woo product pages
                if (function_exists('is_product') && is_product()) {
                    return false;
                }

                // Only pages + blog posts
                if (! is_page() && ! is_single()) {
                    return false;
                }

                // Manual exclude pages
                if (
                    ! empty($rule['exclude_pages_enabled']) &&
                    in_array(
                        $current_page_id,
                        $rule['exclude_pages'] ?? array(),
                        true
                    )
                ) {
                    return false;
                ...
                // Auto skip if page already exists in specific_pages rule
                foreach ($this->rules as $existing_rule) {

                    if (
                        empty($existing_rule['status']) ||
                        'active' !== $existing_rule['status']
                    ) {
                        continue;
                    }

                    if (
                        ($existing_rule['trigger_type'] ?? '') === 'specific_pages'
                        &&
                        in_array(
                            $current_page_id,
                            $existing_rule['pages'] ?? array(),
                            true
                        )
                    ) {
                        return false;
            ...
            case 'all_products':
                ...
                // Manual exclude products
                if (
                    ! empty($rule['exclude_products_enabled']) &&
                    in_array(
                        $current_product_id,
                        $rule['exclude_products'] ?? array(),
                        true
                    )
                ) {
                    return false;
                ...
                // Auto skip if product already exists in specific_products rule
                foreach ($this->rules as $existing_rule) {
                    ...
                    if (
                        ($existing_rule['trigger_type'] ?? '') === 'specific_products'
                        &&
                        in_array(
                            $current_product_id,
                            $existing_rule['products'] ?? array(),
                            true
                        )
                    ) {
                        return false;
