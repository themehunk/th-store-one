jQuery(function ($) {
    ...
    const isStoreBundle = discountScope === 'store_bundle';
    const isStoreProduct = discountScope === 'store_product';
    ...
    function formatPrice(amount) {
        ...
        if (!$tpl.length) return amount.toFixed(2);
        return $tpl.html().replace(/0+([.,]0+)?/, amount.toFixed(2));
    }
    ...
    function updateLinePrice($item) {
        ...
        $item.find('.s1-line-total').html(formatPrice(unit * qty));
    }
    ...
    /* =====================================================
     * AJAX PRICE PREVIEW (GLOBAL SAFE)
     * ===================================================== */
    ...
    window.storeOnePreviewPrice = previewPrice;
    ...
    $(document).on('change', '.s1-variation-form select', function () {
        ...
        $form.find('select').each(function () {
            if (this.value) attrs[this.name] = this.value;
        });

        if (!variations || Object.keys(attrs).length !== $form.find('select').length) return;
        ...
        $item
            .attr('data-variation-id', matched.variation_id)
            .data('variation-id', matched.variation_id)
            .data('variation-attrs', matched.attributes)
            .attr('data-price', matched.display_price);
        ...
        requestAnimationFrame(previewPrice);
    });
    ...
    $('.s1-bundle-item').each(function () {
        const $item = $(this);
        let qty = clampQty($item, getQty($item));
        $item.attr('data-qty', qty);
        updateLinePrice($item);
    });
...
//     if (window.storeOnePreviewPrice) {
//         window.storeOnePreviewPrice();
//     }
// });
// });
